...
type
  ...
  private
    ...
    function GetTouchCoord(const touches: NSSet; var x, y: single; const AView: UIView = nil): Boolean;
...
uses
  System.RTLConsts, System.Math.Vectors, Macapi.ObjCRuntime, FMX.Helpers.iOS, FMX.DialogService,
    ...
    case gestureRecognizer.state of
      ...
      begin
        ...
        begin
          ...
          if (TGridOption.ColumnResize in Model.Options) and
            (Min(Abs(R.Left - TouchPoint.X), Abs(R.Right - TouchPoint.X)) < Min(R.Width / 3, R.Height)) then
          begin
  ...
  begin
    ...
    begin
      ...
      if TouchPoint.x > (LFrame.origin.x + LFrame.size.width / 2)  then
        Inc(I);
      if not InRange(I, FIndexMoveHeader, FIndexMoveHeader + 1) and (I < FHeaderItems.Count) then
...
function TiOSTable.GetTouchCoord(const touches: NSSet; var x, y: single; const AView: UIView = nil): Boolean;
var
  Touch: UITouch;
  P: CGPoint;
begin
...
begin
  try
    ...
    try
      if Model.Enabled and GetTouchCoord(touches, X, Y) then
      begin
        LCoord.Row := -1;
        LCoord.Col := -1;
        if (iOSGrid.Control <> nil) and iOSGrid.Control.GetInterface(IGestureControl, GestureControl) and
          GetTouchCoord(touches, TapPoint.X, TapPoint.Y, iOSGrid.View) then
        begin
        ...
        begin
          ...
          if (LCoord.Row >= 0) and (LCoord.Col >= 0) and (LCoord.Col < Model.ColumnCount) and
            (Model.Columns[LCoord.Col].Visible) and (Model.Columns[LCoord.Col].Enabled) then
          begin
...
begin
  ...
  Result.size.Height := FHeader.View.Frame.size.Height;
end;
...
begin
  ...
  if Editing and (FInplaceEditor <> nil) and (FInplaceEditor.InternalCell <> nil) then
    Result := Model.Columns[Model.Col].PostEditorValue(nil, FInplaceEditor.InternalCell.Data)
  else
    Result := False;
end;
