...
type
  ...
  private
    ...
    function GetCellControl: TFmxObject;
    ...
    function UpdateReadOnly(const Obj: TFmxObject): Boolean;
    { IReadOnly }
  ...
  protected
    ...
    property CellControl: TFmxObject read GetCellControl;
    /// <summary> Interface <b>IDrawableCell</b> from the object <b>CellControl</b> if it exists, and
  ...
  public
    ...
    /// </returns>
    function RefreshEditor(const InplaceEdit: TFmxObject; const Value: TValue): Boolean; virtual;
    ...
    function EditingDone(const InplaceEdit: TFmxObject; const Value: TValue; const Accept: Boolean): Boolean;
    function PostEditorValue(const InplaceEdit: TFmxObject; const Value: TValue): Boolean; virtual;
    ...
    /// <summary> This method place source rectangle in bouns rectangle and uses alignment parameters of text settings
    /// </summary>
  ...
  public
    ...
    /// <summary> ObtaiIndexOfColumnn the coordinates of the rectangle for the cell which is specified by row <b>ARow</b> and column
    ...
    /// coordinatesIndexOfColumn of the top left corner of the returned rectangle will correspond to the next one to the last cell
    /// </para>
...
begin
  ...
  begin
    TComponent(FCellControl).RemoveFreeNotification(Self);
    FCellControl.DisposeOf;
    FCellControl := nil;
  end;
  inherited;
end;
...
begin
  if not FEditorStateChanging and not Released and (Model <> nil) and (Model.Cache <> nil) and (ARow >= 0) and
...
begin
  inherited;
  if (Operation = opRemove) and (AComponent = FCellControl) then
  begin
    FCellControl := nil;
    FDrawableCell := nil;
  end;
...
function TColumn.GetCellControl: TFmxObject;
var
  Info: TGridModel.TCellObjectInfo;
begin
  if FNeedCreateCellControl and (FCellControl = nil) and not Released then
  begin
    Info.Column := Self;
    Info.CellObject := nil;
    FNeedCreateCellControl := False;
    Model.SendMessageWithResult<TGridModel.TCellObjectInfo>(PM_CREATE_CELL_OBJECT, Info);
    FCellControl := Info.CellObject;
    if FCellControl <> nil then
      TComponent(FCellControl).FreeNotification(Self);
    Supports(FCellControl, IDrawableCell, FDrawableCell);
    UpdateReadOnly(FCellControl);
  end;
  Result := FCellControl;
end;
...
function TColumn.RefreshEditor(const InplaceEdit: TFmxObject; const Value: TValue): Boolean;
var
  LReadOnly: Boolean;
begin
  LReadOnly := FReadOnly;
...
function TColumn.PostEditorValue(const InplaceEdit: TFmxObject; const Value: TValue): Boolean;
var
  LCoord: TGridModel.TCoord;
  LinkEditing: Boolean;
begin
...
begin
  if CellControl is TStyledControl then
    TStyledControl(CellControl).ApplyStyleLookup;
...
function TColumn.UpdateReadOnly(const Obj: TFmxObject): Boolean;
var
  LReadOnly: IReadOnly;
begin
  Result := Supports(Obj, IReadOnly, LReadOnly);
  if Result then
...
procedure TCurrencyColumn.BeforeDrawing(const Canvas: TCanvas);
begin
  inherited;
  Layout.HorizontalAlign := TTextAlign.Trailing;
end;

procedure TCurrencyColumn.SetDecimalDigits(const Value: Integer);
begin
...
function TCurrencyColumn.ValueToString(const Value: TValue): string;
var
  D: Double;
begin
  if ValueIsEmpty(Value) then
    Result := string.Empty
  else if ((Value.Kind = TTypeKind.tkUString) and TryStrToFloat(Value.AsString, D)) or Value.TryAsType<Double>(D) then
    Result := Format(Format('%%1.%df', [FDecimalDigits]), [D])
  else
    Result := inherited;
end;
...
begin
  Result := nil;
  if (Column.CellControl is TControl) and (TControl(Column.CellControl).Scene <> nil) then
    Scale := TControl(Column.CellControl).Scene.GetSceneScale
...
begin
  ...
  begin
    ...
    try
      ...
      if (Owner is TControl) and not TControl(Owner).Released then
      begin
...
begin
  inherited;
  if (Model <> nil) and not Released then
    Model.EndUpdate;
end;
...
TColumnClasses.Register([TColumn, TStringColumn, TProgressColumn, TCheckColumn, TDateColumn, TTimeColumn, TPopupColumn,
  TImageColumn, TCurrencyColumn, TGlyphColumn]);
