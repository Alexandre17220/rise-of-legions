...
{ IFMXWBService }
  ...
  private
    procedure FormHandleDestroyed(const Sender: TObject; const Msg: TMessage);
...
begin
  ...
  TMessageManager.DefaultManager.SubscribeToMessage(TBeforeDestroyFormHandle, FormHandleDestroyed);
  CanFocus := True;
  FSavedVisible := True;
  EnableCaching := True;
...
begin
  ...
  TMessageManager.DefaultManager.Unsubscribe(TBeforeDestroyFormHandle, FormHandleDestroyed);
