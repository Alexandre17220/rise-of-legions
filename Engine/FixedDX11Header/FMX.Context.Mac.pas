...
uses
  ...
  Macapi.ObjectiveC, Macapi.Foundation, Macapi.Helpers, FMX.Consts, FMX.Utils, FMX.Platform, FMX.Forms, FMX.Platform.Mac;
...
begin
  if (TCustomContextOpenGL.GlobalBeginSceneCount = 0) and
    (TNSOpenGLContext.OCClass.currentContext <> NSObjectToID(TCustomContextOpenGL.SharedContext)) then
    TCustomContextOpenGL.SharedContext.makeCurrentContext;
end;
...
begin
  ...
  Attributes := [NSOpenGLPFADoubleBuffer];
  if ColorBits <> 0 then
...
begin
  SetCurrentContext;
  Result := (FSharedContext <> nil) and ((Texture <> nil) and (FFrameBuf <> 0)) or (FContextObject <> nil);
end;
...
begin
  ...
  begin
    ...
    else if FContextObject <> nil then
