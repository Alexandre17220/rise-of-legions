...
procedure TBitmapCtx.ContextBeforeLosingHandler(const Sender: TObject; const Msg: TMessage);
begin
{$IFNDEF ANDROID}
  if (FTexture = nil) and (FRenderTarget <> nil) and (FBytesPerLine > 0) and (FCanvas <> nil) and
     (FCanvas.Context <> nil) then
  begin
    CreateBuffer;
    FCanvas.Context.CopyToBits(FData, FBytesPerLine, TRect.Create(0, 0, FWidth, FHeight));
  end;
{$ENDIF}
end;
...
begin
  ...
  begin
    ...
    FTexture.Style := [TTextureStyle.Dynamic, TTextureStyle.Volatile];
    if FData <> nil then
